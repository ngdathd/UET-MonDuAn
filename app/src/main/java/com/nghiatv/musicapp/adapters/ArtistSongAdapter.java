...
import com.nghiatv.musicapp.dataloaders.ArtistAlbumLoader;
                    ...
                    public boolean onMenuItemClick(MenuItem item) {
                        switch (item.getItemId()) {
                            case R.id.popup_song_play:
                                MusicPlayer.playAll(mContext, songIDs, position + 1, -1, MusicUtils.IdType.NA, false);
                                break;
                            ...
                            case R.id.popup_song_share:
                                MusicUtils.shareTrack(mContext, arraylist.get(position + 1).id);
                                break;
                            case R.id.popup_song_delete:
                                long[] deleteIds = {arraylist.get(position + 1).id};
                                MusicUtils.showDeleteDialog(mContext,arraylist.get(position + 1).title, deleteIds, ArtistSongAdapter.this, position + 1);
                                break;
                        }
                        return false;
                    }
            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    playAll(mContext, songIDs, getAdapterPosition(), artistID,
                            MusicUtils.IdType.Artist, false,
