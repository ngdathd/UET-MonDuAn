...
public class LastFMManager extends AsyncTask<Void, Void, Void> {
    ...
    private static final String API_KEY = "e1ee0c040d51c2669ccdc0530144bd63";
    ...
    public LastFMManager(Context context, boolean is_get_song, boolean is_recommendation, boolean is_search, boolean is_youtube_rec) {
        ...
        if (VideosFragment.getSearch_text() != null) {
            search = VideosFragment.getSearch_text().getText().toString();
        }
    ...
    private void youtube_recommend_artist_search() {
        try {
            String sUrl = "http://ws.audioscrobbler.com/2.0/?method=artist.getSimilar&artist=" + URLEncoder.encode(VideosFragment.getArtist_selected(), "UTF-8") + "&limit=20&api_key=" + API_KEY + "&format=json";
            json_string = Helper.try_connection(sUrl);
            JSONArray artists = get_json_array_artists();
            Log.d(TAG, "youtube_recommend_artist_search: " + sUrl);
            ...
            Log.println(Log.ERROR, "LastFMManager:", "Erro no JSON - VideosFragment recommendation");
        }
    }


    private void update_youtube_recommendations() {
        VideosFragment.getArtist_name_list().clear();
        VideosFragment.getImage_artist_list().clear();
        VideosFragment.getArtist_name_list().addAll(this.artist_name_list);
        VideosFragment.getImage_artist_list().addAll(this.image_artist_list);
        VideosFragment.getRecyclerView().getAdapter().notifyDataSetChanged();
    }
    ...
    private void update_search_adapters() {
        VideosFragment.getSearch_text().setAdapter(null);
        ArrayAdapter adapter = new ArrayAdapter(context, R.layout.dropdown, search_list);
        VideosFragment.getSearch_text().setThreshold(3);
        VideosFragment.getSearch_text().setAdapter(adapter);
    }
