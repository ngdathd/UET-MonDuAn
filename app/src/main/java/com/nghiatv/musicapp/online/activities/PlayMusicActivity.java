...
import com.nghiatv.musicapp.online.adapters.ViewPagerPlayMusicAdapter;
import com.nghiatv.musicapp.dto.SongDto;
import com.nghiatv.musicapp.online.fragments.FragmentCDMusic;
import com.nghiatv.musicapp.online.fragments.FragmentPlayDanhSachBaiHat;
...
public class PlayMusicActivity extends AppCompatActivity {
    ...
    public static ArrayList<SongDto> baiHatList = new ArrayList<SongDto>();
...
//khởi tạo handler
        ...
        mMediaPlayer = MediaPlayer.create(this, Uri.parse(baiHatList.get(0).getLink()));
        mMediaPlayer.start();
        //set title action bar
        getSupportActionBar().setTitle(baiHatList.get(0).getTitle());
        TimeSong();
        UpdateTime();
            ...
            public void run() {
                if (mViewPagerPlayMusicAdapter.getItem(1) != null) {
                    if (baiHatList.size() > 0) {
                        mFragmentCDMusic.Playnhac(baiHatList.get(position).getImage());
...
//                    mFragmentCDMusic.stopAnimation();
            ...
            public void onClick(View v) {
                if (baiHatList.size() > 0) {
                    ...
                    if (position < (baiHatList.size())) {
                        ...
                        new PlayMusic().execute(baiHatList.get(position).getLink());
                        mFragmentCDMusic.Playnhac(baiHatList.get(position).getImage());
                        getSupportActionBar().setTitle(baiHatList.get(position).getTitle());
                        UpdateTime();
                    }
                }
    ...
    private void GetDataFromItent() {
        ...
        if (intent != null) {
            if (intent.hasExtra("cakhuc")) {
                SongDto songDto = intent.getParcelableExtra("cakhuc");
                baiHatList.add(songDto);
            }
            if (intent.hasExtra("allbaihat")) {
                ArrayList<SongDto> allbaihatList = intent.getParcelableArrayListExtra("allbaihat");
    ...
    private void init() {
        ...
        if (baiHatList.size() > 0) {
            getSupportActionBar().setTitle(baiHatList.get(0).getTitle());
            new PlayMusic().execute(baiHatList.get(0).getLink());
            btnPlay.setImageResource(R.drawable.iconpause);
        }
    }
